...
function ProductThumb({ row }: { row: InventoryRow }) {
  ...
  const styles: Record<ProductKind, string> = {
    battery: "bg-slate-100 text-slate-900",
    inverter: "bg-sky-50 text-sky-700",
    rack: "bg-zinc-100 text-zinc-800",
    cable: "bg-orange-50 text-orange-700",
    clamp: "bg-slate-100 text-slate-600",
    panel: "bg-blue-50 text-blue-800",
    default: "bg-emerald-50 text-emerald-700",
  };
  const labels: Record<ProductKind, string> = { battery: "BAT", inverter: "INV", rack: "RCK", cable: "CBL", clamp: "CLP", panel: "SOL", default: "SKU" };

  return (
    <div className={cn("flex h-14 w-14 shrink-0 items-center justify-center rounded-2xl border border-slate-200 text-xs font-black shadow-sm", styles[kind])}>
      {labels[kind]}
    </div>
  );
}
  ...
  return (
    <div className="rounded-[1.5rem] border border-slate-200/80 bg-white p-5 shadow-[0_14px_34px_rgba(15,23,42,0.05)]">
      <div className="flex items-start gap-4">
        <div className={cn("flex h-14 w-14 shrink-0 items-center justify-center rounded-full", tone)}><Icon type={icon} /></div>
        <div className="min-w-0"><p className="text-sm font-semibold text-slate-500">{title}</p><p className="mt-1 text-3xl font-bold tracking-tight text-slate-950">{value}</p><p className="mt-1 text-xs font-medium text-slate-500">{subtitle}</p></div>
      </div>
    </div>
  );
}
...
export default function InventoryPage() {
  ...
  const summary = useMemo(() => {
    ...
    return { totalSkus: rows.length, actualOnHand, sellable, incoming, lowStock };
  }, [rows]);
