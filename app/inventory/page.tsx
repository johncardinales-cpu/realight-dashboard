...
function Icon({ type }: { type: "box" | "layers" | "tag" | "truck" | "warning" | "search" | "download" }) {
  ...
  if (type === "download") return <svg viewBox="0 0 24 24" className={common} fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round"><path d="M12 3v12" /><path d="m7 10 5 5 5-5" /><path d="M5 21h14" /></svg>;
  if (type === "truck") return <svg viewBox="0 0 24 24" className={common} fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><path d="M10 17h4V5H2v12h3" /><path d="M14 9h4l4 4v4h-2" /><circle cx="7.5" cy="17.5" r="2.5" /><circle cx="17.5" cy="17.5" r="2.5" /></svg>;
  if (type === "warning") return <svg viewBox="0 0 24 24" className={common} fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><path d="M12 9v4" /><path d="M12 17h.01" /><path d="M10.29 3.86 1.82 18a2 2 0 0 0 1.71 3h16.94a2 2 0 0 0 1.71-3L13.71 3.86a2 2 0 0 0-3.42 0Z" /></svg>;
  if (type === "tag") return <svg viewBox="0 0 24 24" className={common} fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><path d="m20.6 13.4-7.2 7.2a2 2 0 0 1-2.8 0L3 13V3h10l7.6 7.6a2 2 0 0 1 0 2.8Z" /><path d="M7.5 7.5h.01" /></svg>;
  if (type === "layers") return <svg viewBox="0 0 24 24" className={common} fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><path d="m12 3 8 4-8 4-8-4 8-4Z" /><path d="m4 12 8 4 8-4" /><path d="m4 17 8 4 8-4" /></svg>;
...
function ProductArtwork({ kind }: { kind: ProductKind }) {
  if (kind === "panel") return <svg viewBox="0 0 48 48" className="h-12 w-12"><rect x="9" y="8" width="30" height="34" rx="3" fill="#0f2a4f" /><path d="M15 8v34M24 8v34M33 8v34M9 17h30M9 26h30M9 35h30" stroke="#e0f2fe" strokeWidth="1.4" /><path d="M14 5h20" stroke="#f59e0b" strokeWidth="2" strokeLinecap="round" /></svg>;
  if (kind === "cable") return <svg viewBox="0 0 48 48" className="h-12 w-12"><ellipse cx="24" cy="16" rx="15" ry="7" fill="#ef4444" /><path d="M9 16v16c0 4 7 7 15 7s15-3 15-7V16" fill="#dc2626" /><ellipse cx="24" cy="32" rx="15" ry="7" fill="#f97316" /><ellipse cx="24" cy="16" rx="8" ry="3.5" fill="#7f1d1d" /><path d="M39 27c5 1 6 5 2 8" stroke="#111827" strokeWidth="2" fill="none" strokeLinecap="round" /></svg>;
  if (kind === "clamp") return <svg viewBox="0 0 48 48" className="h-12 w-12"><path d="M14 15h20l5 8-5 10H14L9 23l5-8Z" fill="#d1d5db" stroke="#64748b" strokeWidth="1.5" /><path d="M18 19h12l3 5-3 5H18l-3-5 3-5Z" fill="#f8fafc" stroke="#94a3b8" strokeWidth="1.2" /><path d="M10 36h28" stroke="#0f172a" strokeWidth="2" strokeLinecap="round" /></svg>;
  if (kind === "rack") return <svg viewBox="0 0 48 48" className="h-12 w-12"><rect x="13" y="6" width="22" height="36" rx="3" fill="#111827" /><rect x="17" y="11" width="14" height="5" rx="1" fill="#374151" /><rect x="17" y="19" width="14" height="5" rx="1" fill="#374151" /><rect x="17" y="27" width="14" height="5" rx="1" fill="#374151" /><path d="M35 10h4M35 34h4" stroke="#f59e0b" strokeWidth="2" strokeLinecap="round" /></svg>;
  if (kind === "inverter") return <svg viewBox="0 0 48 48" className="h-12 w-12"><rect x="14" y="5" width="20" height="38" rx="4" fill="#f8fafc" stroke="#94a3b8" strokeWidth="1.5" /><rect x="18" y="10" width="12" height="9" rx="2" fill="#0f766e" /><circle cx="20" cy="28" r="2" fill="#22c55e" /><circle cx="28" cy="28" r="2" fill="#f59e0b" /><path d="M18 35h12" stroke="#334155" strokeWidth="2" strokeLinecap="round" /></svg>;
  if (kind === "battery") return <svg viewBox="0 0 48 48" className="h-12 w-12"><rect x="12" y="7" width="24" height="34" rx="4" fill="#111827" /><rect x="18" y="4" width="12" height="5" rx="1.5" fill="#334155" /><rect x="16" y="13" width="16" height="7" rx="2" fill="#1f2937" /><path d="M18 29h12M18 34h12" stroke="#22c55e" strokeWidth="2" strokeLinecap="round" /><path d="M32 12v7" stroke="#f59e0b" strokeWidth="2" strokeLinecap="round" /></svg>;
  return <svg viewBox="0 0 48 48" className="h-12 w-12"><rect x="11" y="11" width="26" height="26" rx="5" fill="#e2e8f0" stroke="#94a3b8" strokeWidth="1.5" /><path d="m17 18 7-4 7 4-7 4-7-4ZM17 24l7 4 7-4M17 30l7 4 7-4" stroke="#0f766e" strokeWidth="1.8" fill="none" strokeLinecap="round" strokeLinejoin="round" /></svg>;
}
...
function KpiCard({ title, value, subtitle, tone, icon }: { title: string; value: string; subtitle: string; tone: string; icon: "box" | "layers" | "tag" | "truck" | "warning" }) {
  return <div className="rounded-[1.5rem] border border-slate-200/80 bg-white p-5 shadow-[0_14px_34px_rgba(15,23,42,0.05)]"><div className="flex items-start gap-4"><div className={cn("flex h-14 w-14 shrink-0 items-center justify-center rounded-full", tone)}><Icon type={icon} /></div><div className="min-w-0"><p className="text-sm font-semibold text-slate-500">{title}</p><p className="mt-1 text-3xl font-bold tracking-tight text-slate-950">{value}</p><p className="mt-1 text-xs font-medium text-slate-500">{subtitle}</p></div></div></div>;
}

function StatusBadge({ status }: { status: InventoryStatus }) {
  const styles: Record<InventoryStatus, string> = { "In Stock": "bg-emerald-50 text-emerald-700", "Low Stock": "bg-amber-50 text-amber-700", Incoming: "bg-blue-50 text-blue-700", "Out of Stock": "bg-rose-50 text-rose-700" };
  const dots: Record<InventoryStatus, string> = { "In Stock": "bg-emerald-500", "Low Stock": "bg-amber-500", Incoming: "bg-blue-500", "Out of Stock": "bg-rose-500" };
  return <span className={cn("inline-flex items-center gap-2 rounded-full px-3 py-1 text-xs font-bold", styles[status])}><span className={cn("h-2 w-2 rounded-full", dots[status])} />{status}</span>;
}
...
export default function InventoryPage() {
  ...
  function setField(keyValue: string, field: "Latest Received" | "Latest Incoming", value: string) {
    setRows((prev) => prev.map((row) => rowKey(row) === keyValue ? { ...row, [field]: value } : row));
  }
  ...
  async function saveDates(row: InventoryRow) {
    ...
    try {
      ...
      setMessage(getErrorMessage(error, "Failed to update dates."));
    } finally {
      setSavingKey("");
    }
  }
  ...
  async function markDamaged() {
    ...
    const description = String(damageTarget["Description"] || "");
    const specification = String(damageTarget["Specification"] || "");
    const quantity = Number(damageQty) || 0;
    ...
    try {
      ...
      setMessage(getErrorMessage(error, "Failed to mark item damaged."));
    } finally {
      setDamagingKey("");
    }
  }
  ...
  const sortedRows = useMemo(() => [...rows].sort((a, b) => {
    const aPrimary = qty(a, "Incoming Qty") > 0 ? parseDateValue(String(a["Latest Incoming"] || "")) : parseDateValue(String(a["Latest Received"] || ""));
    const bPrimary = qty(b, "Incoming Qty") > 0 ? parseDateValue(String(b["Latest Incoming"] || "")) : parseDateValue(String(b["Latest Received"] || ""));
  ...
  const summary = useMemo(() => {
    ...
    const damaged = rows.reduce((total, row) => total + qty(row, "Damaged Qty"), 0);
    const lowStock = rows.filter((row) => ["Low Stock", "Out of Stock"].includes(getStatus(row))).length;
    return { totalSkus: rows.length, actualOnHand, sellable, incoming, damaged, lowStock };
  }, [rows]);
