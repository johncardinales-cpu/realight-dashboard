...
export default function ReportsPageClient() {
  ...
  const openReceivables = useMemo(() => {
    ...
    return rows
      ...
      .sort((a, b) => `${a.saleDate}-${a.salesRefNo}`.localeCompare(`${b.saleDate}-${b.salesRefNo}`));
  }, [payments, data]);
  ...
  return <section className="space-y-6">
    <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
      <div className="flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between">
        <div><h1 className="text-3xl font-semibold text-slate-900">Reports</h1><p className="mt-1 text-sm text-slate-600">Sales use Sale Date. Collections from prior receivables are separated for audit.</p>{data ? <p className="mt-2 text-sm font-semibold text-slate-800">{title(data.mode)} Report: {rangeLabel}</p> : null}</div>
        <form className="flex flex-wrap gap-3" onSubmit={(event) => { event.preventDefault(); loadReport().catch(console.error); }}>
          <select className="rounded-xl border border-slate-300 px-3 py-2" value={mode} onChange={(event) => setMode(event.target.value as Mode)}>{reportModes.map((item) => <option key={item.value} value={item.value}>{item.label}</option>)}</select>
          <input className="rounded-xl border border-slate-300 px-3 py-2" type="date" value={date} onChange={(event) => setDate(event.target.value)} disabled={mode === "overall"} />
          <button className="rounded-xl bg-emerald-600 px-5 py-2 text-sm font-bold text-white" disabled={loading}>{loading ? "Loading..." : "Load Report"}</button>
        </form>
      </div>
    ...
    <div className="grid gap-6 xl:grid-cols-2">
      <Section title="Open Receivables" helper="Shows all unpaid balances, not only sales created inside the selected report period."><div className="overflow-x-auto rounded-2xl border border-slate-200"><table className="min-w-full text-left text-sm"><thead className="bg-slate-50 text-slate-500"><tr><th className="px-4 py-3">Date</th><th className="px-4 py-3">Sales Ref</th><th className="px-4 py-3">Customer</th><th className="px-4 py-3 text-right">Total</th><th className="px-4 py-3 text-right">Paid</th><th className="px-4 py-3 text-right">Balance</th><th className="px-4 py-3">Status</th></tr></thead><tbody>{openReceivables.length ? openReceivables.map((sale) => <tr key={`${sale.salesRefNo}-${sale.customerName}-${sale.saleDate}`} className="border-t border-slate-100"><td className="px-4 py-3">{sale.saleDate}</td><td className="px-4 py-3">{sale.salesRefNo}</td><td className="px-4 py-3">{sale.customerName}</td><td className="px-4 py-3 text-right">{money(sale.totalSalePhp)}</td><td className="px-4 py-3 text-right">{money(sale.totalPaidPhp)}</td><td className="px-4 py-3 text-right font-bold">{money(sale.balancePhp)}</td><td className="px-4 py-3"><Pill value={sale.paymentStatus} /></td></tr>) : <tr><td className="px-4 py-6 text-center text-slate-500" colSpan={7}>No open receivables.</td></tr>}</tbody></table></div></Section>
      <Section title="Expense Detail"><div className="overflow-x-auto rounded-2xl border border-slate-200"><table className="min-w-full text-left text-sm"><thead className="bg-slate-50 text-slate-500"><tr><th className="px-4 py-3">Date</th><th className="px-4 py-3">Category</th><th className="px-4 py-3">Description</th><th className="px-4 py-3 text-right">Amount</th><th className="px-4 py-3">Source</th></tr></thead><tbody>{data?.dailyExpenses.length ? data.dailyExpenses.map((expense) => <tr key={`${expense.date}-${expense.category}-${expense.description}`} className="border-t border-slate-100"><td className="px-4 py-3">{expense.date}</td><td className="px-4 py-3">{expense.category}</td><td className="px-4 py-3">{expense.description}</td><td className="px-4 py-3 text-right font-bold">{money(expense.amount)}</td><td className="px-4 py-3">{expense.source}</td></tr>) : <tr><td className="px-4 py-6 text-center text-slate-500" colSpan={5}>No expenses for this period.</td></tr>}</tbody></table></div></Section>
    </div>
  </section>;
}
