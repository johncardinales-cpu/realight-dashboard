...
type ReportsData = {
  ...
  dailyTrend: Trend[];
  productMovement: Array<{ description: string; qty: number; confirmedQty: number }>;
};
...
function peso(value: number | undefined) {
...
function MiniChart({ rows }: { rows: Trend[] }) {
  const max = Math.max(...rows.map((r) => Math.max(r.sales || 0, r.grossProfit || 0, r.collections || 0, r.expenses || 0, r.netProfit || 0)), 1);
  const hasData = rows.some((r) => Number(r.sales || 0) > 0 || Number(r.grossProfit || 0) > 0 || Number(r.collections || 0) > 0 || Number(r.expenses || 0) > 0);

  if (!rows.length) {
    return (
      <div className="flex h-[240px] items-center justify-center rounded-2xl border border-dashed border-slate-200 bg-slate-50 text-center">
        <div><p className="text-lg font-bold text-slate-950">No date range selected</p><p className="mt-2 text-sm text-slate-500">Choose dates and click Apply.</p></div>
      </div>
    );
  }
  ...
  const bars = [
    ...
    { key: "grossProfit", label: "Gross Profit", color: "bg-violet-500" },
    { key: "expenses", label: "Expenses", color: "bg-orange-500" },
  ] as const;
  ...
  return (
    <div className="rounded-2xl border border-slate-100 bg-slate-50 p-5">
      ...
      <div className="flex h-[220px] items-end gap-4 overflow-x-auto pb-2">
        {rows.map((r) => (
          <div key={r.date} className="flex min-w-[90px] flex-1 flex-col items-center justify-end gap-2">
            <div className="flex h-[170px] items-end gap-1.5">
              {bars.map((bar) => <div key={bar.key} className={`w-4 rounded-t-lg ${bar.color}`} style={{ height: `${Math.max((Number(r[bar.key] || 0) / max) * 170, hasData ? 2 : 0)}px` }} />)}
            </div>
      ...
      <div className="mt-3 flex flex-wrap gap-4 text-xs font-semibold text-slate-500">
        {bars.map((bar) => <span key={bar.key} className="inline-flex items-center gap-2"><span className={`h-3 w-3 rounded ${bar.color}`} />{bar.label}</span>)}
      </div>
    </div>
  );
}
...
export default function HomePage() {
  ...
  const kpis = useMemo(() => [
    ...
    { label: "Low Stock", value: num(lowStockItems), change: lowStockItems ? "Needs review" : "Clear", helper: "live inventory", tone: "orange", icon: icons.orders },
  ...
  return (
    <section className="space-y-6">
      <div className="flex items-start justify-between gap-4">
        <div><h1 className="text-3xl font-bold tracking-tight text-slate-950">Dashboard</h1><p className="mt-1 text-sm font-medium text-slate-500">Live business snapshot from the same totals used in Reports.</p></div>
        <button onClick={() => loadDashboard(rangeStart, rangeEnd).catch(console.error)} className="rounded-xl border border-slate-200 bg-white px-4 py-2 text-sm font-bold text-slate-700 shadow-sm">{loading ? "Refreshing..." : "Refresh"}</button>
      </div>

      <div className="grid grid-cols-1 gap-5 md:grid-cols-2 xl:grid-cols-4">
        {kpis.map((item) => (
          <div key={item.label} className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
            <div className="flex items-center gap-4"><IconCircle icon={item.icon} tone={item.tone} /><div><p className="text-sm font-semibold text-slate-500">{item.label}</p><p className="mt-2 text-2xl font-bold tracking-tight text-slate-950">{item.value}</p><p className={`mt-2 text-sm font-bold ${item.tone === "rose" ? "text-rose-600" : item.tone === "orange" ? "text-orange-600" : "text-emerald-600"}`}>{item.change}</p><p className="mt-1 text-sm text-slate-500">{item.helper}</p></div></div>
          </div>
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 gap-5 xl:grid-cols-[minmax(0,1.15fr)_minmax(420px,0.85fr)]">
        <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
          <div className="mb-5 flex flex-wrap items-center justify-between gap-3">
            <div><h2 className="text-lg font-bold text-slate-950">Sales Overview</h2><p className="mt-1 text-xs font-semibold text-slate-500">Showing {rangeStart} to {rangeEnd}</p></div>
            <div className="flex flex-wrap items-center gap-2">
              <div className="flex rounded-xl border border-slate-200 bg-white p-1 shadow-sm">
                {(["daily", "weekly", "monthly", "yearly", "custom"] as PeriodMode[]).map((mode) => <button key={mode} onClick={() => selectPreset(mode)} className={`rounded-lg px-3 py-1.5 text-sm font-bold ${periodMode === mode ? "bg-emerald-600 text-white" : "text-slate-600 hover:bg-slate-50"}`}>{title(mode)}</button>)}
              </div>
              <div className="flex flex-wrap items-center gap-2 rounded-xl border border-slate-200 bg-slate-50 p-2"><input type="date" value={rangeStart} onChange={(e) => { setRangeStart(e.target.value); setPeriodMode("custom"); }} className="rounded-lg border border-slate-200 bg-white px-2 py-1.5 text-xs font-semibold text-slate-700" /><span className="text-xs font-bold text-slate-400">to</span><input type="date" value={rangeEnd} onChange={(e) => { setRangeEnd(e.target.value); setPeriodMode("custom"); }} className="rounded-lg border border-slate-200 bg-white px-2 py-1.5 text-xs font-semibold text-slate-700" /><button onClick={applyCustomRange} className="rounded-lg bg-emerald-600 px-3 py-1.5 text-xs font-bold text-white">Apply</button></div>
            </div>
          </div>
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
          <div className="mb-5 flex items-center justify-between"><h2 className="text-lg font-bold text-slate-950">Recent Activities</h2><button onClick={() => go("/reports")} className="rounded-xl border border-slate-200 bg-white px-4 py-2 text-sm font-bold text-slate-700 shadow-sm">View All</button></div>
          {activities.length ? <div className="divide-y divide-slate-100">{activities.slice(0, 5).map((item) => <div key={item.id || item.title} className="flex items-center gap-4 py-4 first:pt-0 last:pb-0"><span className="flex h-12 w-12 shrink-0 items-center justify-center rounded-full bg-emerald-50 text-emerald-600"><svg viewBox="0 0 24 24" className="h-6 w-6" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d={icons.activity} /></svg></span><div className="min-w-0 flex-1"><p className="font-bold text-slate-950">{item.title}</p><p className="text-sm text-slate-500">{item.note}</p></div><p className="text-sm text-slate-500">{item.time}</p></div>)}</div> : <div className="flex h-[240px] items-center justify-center rounded-2xl border border-dashed border-slate-200 bg-slate-50 text-center"><div><p className="font-bold text-slate-950">No recent activities</p><p className="mt-1 text-sm text-slate-500">Activity will appear after testing begins.</p></div></div>}
        </div>
      </div>
