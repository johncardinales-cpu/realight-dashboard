...
type InventoryRow = { Description: string; Specification: string; "Sellable Qty": number; "Actual On Hand"?: number };
...
export default function SalesPage() {
  ...
  async function loadAll() {
    const [salesRes, pricingRes, customersRes, paymentsRes, inventoryRes] = await Promise.all([
  ...
  useEffect(() => {
    const storedSalesperson = window.localStorage.getItem(salespersonStorageKey) || "";
    if (storedSalesperson) {
      setSalesperson(storedSalesperson);
      setCashierName(storedSalesperson);
    }
  }, []);
  ...
  const stockMap = useMemo(() => { const map = new Map<string, number>(); inventory.forEach((row) => map.set(itemKey(row.Description, row.Specification), Number(row["Sellable Qty"]) || 0)); return map; }, [inventory]);
  const requestedQtyMap = useMemo(() => { const map = new Map<string, number>(); items.forEach((item) => { const key = itemKey(item.description, item.specification); if (key === "|||") return; map.set(key, (map.get(key) || 0) + (Number(item.qty) || 0)); }); return map; }, [items]);
  const stockIssue = useMemo(() => { const problems: string[] = []; requestedQtyMap.forEach((requested, key) => { const available = stockMap.get(key) ?? 0; if (requested > available) { const [description, specification] = key.split("|||"); problems.push(`${description} / ${specification}: requested ${requested}, available ${available}`); } }); return problems.join("; "); }, [requestedQtyMap, stockMap]);
  const range = useMemo(() => periodRange(periodMode, periodDate), [periodMode, periodDate]);
  ...
  function updateSalesperson(value: string) {
    const cleanValue = value.trimStart();
    setSalesperson(cleanValue);
    setCashierName(cleanValue);
    window.localStorage.setItem(salespersonStorageKey, cleanValue);
  }
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    setAlert("");
    try {
      const cleanItems = items.filter((x) => x.description && x.specification && Number(x.qty) > 0);
      if (!cleanItems.length) throw new Error("Add at least one valid product line.");
      ...
      const finalSalesperson = salesperson.trim();
      const finalCashierName = cashierName.trim() || finalSalesperson;
      ...
      setAlert(`Sale saved. ${data.lines || cleanItems.length} line(s).`);
      resetForm();
      await loadAll();
    } catch (error: any) {
      setAlert(error?.message || "Failed to save sale.");
    } finally {
      setSaving(false);
    }
  }
    ...
    <section className="space-y-6">
      ...
      <form onSubmit={onSubmit} className="space-y-5 rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
        <div className="grid gap-4 md:grid-cols-3">
          ...
          <Field label="Salesperson" helper="This also records the cashier/operator for the sale."><input className={inputClass} value={salesperson} onChange={(e) => updateSalesperson(e.target.value)} placeholder="Salesperson name" /></Field>
        </div>
