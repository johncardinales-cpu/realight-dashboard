...
function balanceFromOrders(unpaidOrders: UnpaidOrder[]) {
  return unpaidOrders.reduce((sum, order) => sum + (Number(order.balancePhp) || 0), 0);
}

function customerBalance(row: CustomerRow, unpaidOrders?: UnpaidOrder[]) {
  const fromCustomerSummary = Number(row.outstandingBalancePhp) || 0;
  const fromVisibleOrders = balanceFromOrders(unpaidOrders || unpaidOrdersFor(row));
  return Math.max(fromCustomerSummary, fromVisibleOrders);
}
...
export default function CustomersPage() {
  ...
  const totalReceivables = rows.reduce((sum, row) => sum + customerBalance(row), 0);
  ...
  return (
    <section className="space-y-6">
      <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
        ...
        <p className="mt-2 max-w-4xl text-sm leading-6 text-slate-600">Keep live customer records clean for sales history, receivables, warranty, and repeat orders.</p>
        ...
        <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
          ...
          <div className="mt-4 max-h-[70vh] overflow-y-auto rounded-2xl border border-slate-200">
            {filteredRows.map((row) => {
              ...
              const totalUnpaid = customerBalance(row, unpaidOrders);
              const hasBalance = totalUnpaid > 0;
                  ...
                  <div className="flex flex-col gap-3 px-4 py-3 lg:flex-row lg:items-center lg:justify-between">
                    <div className="min-w-0">
                      <div className="flex flex-wrap items-center gap-2">
                        ...
                        {hasBalance ? <span className="rounded-full bg-rose-50 px-2.5 py-1 text-[11px] font-bold text-rose-700">Unpaid</span> : null}
                      </div>
                    ...
                    <div className="flex flex-wrap items-center gap-2 text-xs lg:justify-end">
                      ...
                      <div className={`${hasBalance ? "border border-rose-100 bg-rose-50" : "border border-emerald-100 bg-emerald-50"} rounded-xl px-3 py-2`}><p className={`text-[11px] font-semibold ${hasBalance ? "text-rose-500" : "text-emerald-600"}`}>{hasBalance ? "Balance Remaining" : "Balance Status"}</p><p className={`font-bold ${hasBalance ? "text-rose-700" : "text-emerald-700"}`}>{hasBalance ? money(totalUnpaid) : "Clear"}</p></div>
                  ...
                  {expanded ? (
                    <div className="border-t border-slate-100 bg-slate-50/60 p-4">
                      <div className="mb-4 grid gap-3 md:grid-cols-4">
                        <div className="rounded-xl border border-slate-200 bg-white p-3"><p className="text-[11px] font-semibold text-slate-500">{hasBalance ? "Balance Remaining" : "Balance Status"}</p><p className={`mt-1 text-base font-bold ${hasBalance ? "text-rose-700" : "text-emerald-700"}`}>{hasBalance ? money(totalUnpaid) : "Clear"}</p></div>
