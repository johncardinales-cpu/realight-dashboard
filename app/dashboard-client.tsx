...
function title(mode: PeriodMode) { return mode === "daily" ? "Daily" : mode === "weekly" ? "Weekly" : mode === "monthly" ? "Monthly" : mode === "yearly" ? "Yearly" : "Custom"; }
function rangeForMode(mode: PeriodMode, anchorValue = today()) { const anchor = safeDate(anchorValue); const start = new Date(anchor); const end = new Date(anchor); if (mode === "daily") return { start: fmt(start), end: fmt(end) }; if (mode === "weekly") { const day = start.getDay(); const offset = day === 0 ? -6 : 1 - day; start.setDate(start.getDate() + offset); end.setTime(start.getTime()); end.setDate(start.getDate() + 6); return { start: fmt(start), end: fmt(end) }; } if (mode === "monthly") { start.setDate(1); end.setTime(start.getTime()); end.setMonth(start.getMonth() + 1, 0); return { start: fmt(start), end: fmt(end) }; } if (mode === "yearly") { start.setMonth(0, 1); end.setMonth(11, 31); return { start: fmt(start), end: fmt(end) }; } return { start: fmt(start), end: fmt(end) }; }
function dateRange(start: string, end: string) { const a = safeDate(start); const b = safeDate(end); if (a > b) return []; const days: string[] = []; const d = new Date(a); while (d <= b && days.length < 370) { days.push(fmt(d)); d.setDate(d.getDate() + 1); } return days; }
...
function reconcileWithPayments(report: ReportsData, payments: PaymentSummary[], start: string, end: string): ReportsData {
  ...
  const openInPeriod = open.filter((p) => inRange(p.saleDate, start, end));
  ...
  return { ...report, summary: { ...report.summary, newReceivablesToday: round(openInPeriod.reduce((sum, p) => sum + Number(p.balancePhp || 0), 0)), endingReceivables: round(open.reduce((sum, p) => sum + Number(p.balancePhp || 0), 0)) }, dailyTrend: report.dailyTrend.map((row) => ({ ...row, receivables: round(receivablesByDate.get(row.date) || 0) })) };
}

function StatCard({ label, value, status, helper, tone = "emerald" }: { label: string; value: string; status: string; helper: string; tone?: "emerald" | "blue" | "violet" | "orange" | "rose" | "slate" }) {
...
export default function DashboardClient() {
  ...
  async function fetchReports(start: string, end: string): Promise<ReportsData> {
  ...
  async function loadDashboard(start = rangeStart, end = rangeEnd) {
    setLoading(true);
    try {
      const [dashboardRes, reportsData, activityRes, productsRes, paymentsRes] = await Promise.all([fetch(`/api/dashboard?t=${Date.now()}`, { cache: "no-store" }), fetchReports(start, end), fetch(`/api/recent-activity?t=${Date.now()}`, { cache: "no-store" }), fetch(`/api/dashboard/top-products?t=${Date.now()}`, { cache: "no-store" }), fetch(`/api/payments?t=${Date.now()}`, { cache: "no-store" })]);
  ...
  function applyCustomRange() { setPeriodMode("custom"); loadDashboard(rangeStart, rangeEnd).catch(console.error); }
  useEffect(() => { loadDashboard(initialRange.start, initialRange.end).catch(console.error); }, []);
  ...
  const stats = useMemo(() => [{ label: "Sales", value: peso(totalSales), status: totalSales ? "Live" : "Zero state", helper: `${title(periodMode).toLowerCase()} confirmed sales`, tone: "emerald" as const }, { label: "Collections", value: peso(collections), status: collections ? "Collected" : "Zero state", helper: "applied payments", tone: "blue" as const }, { label: "Net Cash", value: peso(netCash), status: cashReceived ? `${peso(cashReceived)} tendered` : "Zero state", helper: `less ${peso(changeGiven)} change`, tone: "emerald" as const }, { label: "Net Profit", value: peso(netProfit), status: expenses ? `${peso(expenses)} expenses` : "No expenses", helper: `${peso(grossProfit)} gross profit`, tone: netProfit < 0 ? "rose" as const : "violet" as const }, { label: "Expenses", value: peso(expenses), status: expenses ? "Recorded" : "Zero state", helper: "manual + supplier costs", tone: expenses ? "orange" as const : "slate" as const }, { label: "Receivables", value: peso(receivables), status: receivables ? "Open balances" : "Clear", helper: "from Payments open balances", tone: receivables ? "orange" as const : "emerald" as const }, { label: "Orders", value: num(totalOrders), status: totalOrders ? "Live" : "Zero state", helper: `${num(unitsSold)} confirmed units sold`, tone: "blue" as const }, { label: "Low Stock", value: num(lowStockItems), status: lowStockItems ? "Needs review" : "Clear", helper: "live inventory", tone: "orange" as const }], [totalSales, collections, netCash, cashReceived, changeGiven, netProfit, expenses, grossProfit, receivables, totalOrders, unitsSold, lowStockItems, periodMode]);

  return <section className="space-y-6"><div className="flex items-start justify-between gap-4"><div><h1 className="text-3xl font-bold tracking-tight text-slate-950">Dashboard</h1><p className="mt-1 text-sm font-medium text-slate-500">Live business snapshot from Reports plus reconciled Payments open balances.</p></div><button onClick={() => loadDashboard(rangeStart, rangeEnd).catch(console.error)} className="rounded-xl border border-slate-200 bg-white px-4 py-2 text-sm font-bold text-slate-700 shadow-sm">{loading ? "Refreshing..." : "Refresh"}</button></div><div className="grid grid-cols-1 gap-5 md:grid-cols-2 xl:grid-cols-4">{stats.map((item) => <StatCard key={item.label} {...item} />)}</div><div className="grid grid-cols-1 gap-5 xl:grid-cols-[minmax(0,1.15fr)_minmax(420px,0.85fr)]"><div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm"><div className="mb-5 flex flex-wrap items-center justify-between gap-3"><div><h2 className="text-lg font-bold text-slate-950">Sales Overview</h2><p className="mt-1 text-xs font-semibold text-slate-500">Showing {rangeStart} to {rangeEnd}</p></div><div className="flex flex-wrap items-center gap-2"><div className="flex rounded-xl border border-slate-200 bg-white p-1 shadow-sm">{(["daily", "weekly", "monthly", "yearly", "custom"] as PeriodMode[]).map((mode) => <button key={mode} onClick={() => selectPreset(mode)} className={`rounded-lg px-3 py-1.5 text-sm font-bold ${periodMode === mode ? "bg-emerald-600 text-white" : "text-slate-600 hover:bg-slate-50"}`}>{title(mode)}</button>)}</div><div className="flex flex-wrap items-center gap-2 rounded-xl border border-slate-200 bg-slate-50 p-2"><input type="date" value={rangeStart} onChange={(event) => { setRangeStart(event.target.value); setPeriodMode("custom"); }} className="rounded-lg border border-slate-200 bg-white px-2 py-1.5 text-xs font-semibold text-slate-700" /><span className="text-xs font-bold text-slate-400">to</span><input type="date" value={rangeEnd} onChange={(event) => { setRangeEnd(event.target.value); setPeriodMode("custom"); }} className="rounded-lg border border-slate-200 bg-white px-2 py-1.5 text-xs font-semibold text-slate-700" /><button onClick={applyCustomRange} className="rounded-lg bg-emerald-600 px-3 py-1.5 text-xs font-bold text-white">Apply</button></div></div></div><MiniChart rows={reports?.dailyTrend || []} /><div className="mt-4 grid grid-cols-2 gap-4 md:grid-cols-5 md:divide-x md:divide-slate-200"><div className="md:px-4 md:first:pl-0"><p className="text-lg font-bold text-slate-950">{peso(totalSales)}</p><p className="text-sm text-slate-500">Sales</p></div><div className="md:px-4"><p className="text-lg font-bold text-slate-950">{peso(collections)}</p><p className="text-sm text-slate-500">Collections</p></div><div className="md:px-4"><p className="text-lg font-bold text-slate-950">{peso(expenses)}</p><p className="text-sm text-slate-500">Expenses</p></div><div className="md:px-4"><p className="text-lg font-bold text-slate-950">{peso(grossProfit)}</p><p className="text-sm text-slate-500">Gross Profit</p></div><div className="md:px-4"><p className={`text-lg font-bold ${netProfit < 0 ? "text-rose-600" : "text-emerald-600"}`}>{peso(netProfit)}</p><p className="text-sm text-slate-500">Net Profit</p></div></div></div><div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm"><div className="mb-5 flex items-center justify-between"><h2 className="text-lg font-bold text-slate-950">Recent Activities</h2><a href="/reports" className="rounded-xl border border-slate-200 bg-white px-4 py-2 text-sm font-bold text-slate-700 shadow-sm">View All</a></div>{activities.length ? <div className="divide-y divide-slate-100">{activities.slice(0, 5).map((item) => <div key={item.id || item.title} className="py-4 first:pt-0 last:pb-0"><p className="font-bold text-slate-950">{item.title}</p><p className="text-sm text-slate-500">{item.note}</p><p className="mt-1 text-xs text-slate-400">{item.time}</p></div>)}</div> : <div className="flex h-[240px] items-center justify-center rounded-2xl border border-dashed border-slate-200 bg-slate-50 text-center"><p className="font-bold text-slate-500">No recent activities</p></div>}</div></div><div className="grid grid-cols-1 gap-5 xl:grid-cols-[minmax(0,0.9fr)_minmax(420px,0.6fr)]"><div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm"><h2 className="text-lg font-bold text-slate-950">Inventory Summary</h2><div className="mt-5 grid grid-cols-2 gap-4 md:grid-cols-4 md:divide-x md:divide-slate-200"><div className="md:px-4 md:first:pl-0"><p className="text-sm text-slate-500">Total Items</p><p className="mt-2 text-2xl font-bold text-slate-950">{num(totalItems)}</p><p className="text-sm text-slate-500">All items in inventory</p></div><div className="md:px-4"><p className="text-sm text-slate-500">In Stock</p><p className="mt-2 text-2xl font-bold text-blue-600">{num(inStock)}</p><p className="text-sm text-slate-500">Items available</p></div><div className="md:px-4"><p className="text-sm text-slate-500">Low Stock</p><p className="mt-2 text-2xl font-bold text-orange-600">{num(lowStockItems)}</p><p className="text-sm text-slate-500">Need attention</p></div><div className="md:px-4"><p className="text-sm text-slate-500">Out of Stock</p><p className="mt-2 text-2xl font-bold text-rose-600">{num(outOfStock)}</p><p className="text-sm text-slate-500">Restock required</p></div></div></div><div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm"><div className="mb-5 flex items-center justify-between"><h2 className="text-lg font-bold text-slate-950">Top Selling Products</h2><a href="/reports" className="rounded-xl border border-slate-200 bg-white px-4 py-2 text-sm font-bold text-slate-700 shadow-sm">View Report</a></div>{topProducts.length ? <div className="space-y-4">{topProducts.slice(0, 5).map((item, index) => <div key={item.name} className="flex items-center justify-between gap-4"><div className="flex items-center gap-3"><span className="flex h-7 w-7 items-center justify-center rounded-full bg-emerald-50 text-xs font-bold text-emerald-700">{index + 1}</span><p className="font-bold text-slate-950">{item.name}</p></div><p className="text-sm font-semibold text-slate-500">{num(item.sold)} sold</p></div>)}</div> : <p className="text-sm text-slate-500">No product sales yet.</p>}</div></div></section>;
}
