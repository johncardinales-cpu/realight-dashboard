...
export default function DashboardClient() {
  ...
  const directSaleCollections = round(Math.max(collections - processedPayments, 0));
  ...
  const stats = useMemo(() => [
    ...
    { label: "Collections", value: peso(collections), status: collections ? "Collected" : "Zero state", helper: "processed + direct sale", tone: "blue" as const },
    { label: "Processed Payments", value: peso(processedPayments), status: processedPayments ? "Encoded" : "Zero state", helper: "payment ledger entries", tone: "blue" as const },
    { label: "Direct Sale Collections", value: peso(directSaleCollections), status: directSaleCollections ? "Sale-time cash" : "Zero state", helper: "collected during sale entry", tone: "emerald" as const },
  ...
  ], [totalSales, collections, processedPayments, directSaleCollections, netCash, cashReceived, changeGiven, netProfit, expenses, grossProfit, receivables, totalOrders, unitsSold, lowStockItems, periodMode]);
