...
async function getPricingMap(sheets: any) {
  ...
  rows.slice(1).forEach((row: string[]) => { const key = itemKey(String(row[1] || ""), String(row[2] || "")); const costPhp = toNumber(row[7]); if (key !== "|||") map.set(key, costPhp); });
  return map;
}
...
async function getAvailableStockMap(sheets: any) {
  ...
  rows.slice(1).forEach((row: string[]) => {
    const description = String(row[4] || "").trim(); const specification = String(row[5] || "").trim(); const qty = toNumber(row[6]); const status = String(row[9] || "").trim().toLowerCase();
    if (!description && !specification) return;
    const key = itemKey(description, specification); const current = map.get(key) || 0;
...
async function getConfirmedSoldMap(sheets: any) {
  ...
  rows.slice(1).filter(isValidSalesRow).forEach((row: string[]) => {
    const description = String(row[3] || "").trim(); const specification = String(row[4] || "").trim(); const qty = toNumber(row[5]); const saleStatus = String(row[20] || "Draft").trim().toLowerCase();
    if (saleStatus !== "confirmed") return;
    const key = itemKey(description, specification); map.set(key, (map.get(key) || 0) + qty);
  });
  return map;
}
...
function getRequestedQtyMap(items: any[]) {
  ...
  items.forEach((item: any) => { const description = String(item?.description || "").trim(); const specification = String(item?.specification || "").trim(); const qty = toNumber(item?.qty); const key = itemKey(description, specification); const current = map.get(key); map.set(key, { description, specification, qty: (current?.qty || 0) + qty }); });
  return map;
}
...
async function validateConfirmedStock(sheets: any, items: any[]) {
  ...
  requestedQtyMap.forEach(({ description, specification, qty }, key) => { const availableQty = Math.max((availableStockMap.get(key) || 0) - (confirmedSoldMap.get(key) || 0), 0); if (qty > availableQty) insufficientItems.push(`${description} / ${specification}: requested ${qty}, available ${availableQty}`); });
...
export async function POST(req: Request) {
  try {
    ...
    const saleDate = String(body?.saleDate || "").trim(); const salesRefNo = String(body?.salesRefNo || "").trim(); const customerId = String(body?.customerId || "").trim(); const customerName = String(body?.customerName || "").trim(); const paymentStatus = String(body?.paymentStatus || "Pending").trim(); const salesperson = String(body?.salesperson || "").trim(); const notes = String(body?.notes || "").trim(); const groupRef = String(body?.groupRef || salesRefNo || `${Date.now()}`).trim(); const paymentMethod = String(body?.paymentMethod || "").trim();
    const tenderedAmountPhp = Math.max(toNumber(body?.amountPaidPhp), 0);
    const transactionRef = String(body?.transactionRef || "").trim(); const cashierName = String(body?.cashierName || "").trim(); const saleStatus = String(body?.saleStatus || "Draft").trim(); const normalizedSaleStatus = saleStatus.toLowerCase(); const createdAt = new Date().toISOString(); const confirmedAt = normalizedSaleStatus === "confirmed" ? String(body?.confirmedAt || createdAt).trim() : String(body?.confirmedAt || "").trim(); const items = Array.isArray(body?.items) ? body.items : [];
    ...
    const deliveryFeePhp = Math.max(toNumber(body?.deliveryFeePhp), 0); const installationFeePhp = Math.max(toNumber(body?.installationFeePhp), 0); const otherChargePhp = Math.max(toNumber(body?.otherChargePhp), 0); const discountPhp = Math.max(toNumber(body?.discountPhp), 0);
    ...
    const taxRatePct = Math.max(toNumber(body?.taxRatePct), 0); const requestedTaxAmountPhp = toNumber(body?.taxAmountPhp); const taxAmountPhp = roundMoney(requestedTaxAmountPhp > 0 ? requestedTaxAmountPhp : taxableBasePhp * (taxRatePct / 100));
    ...
    if (normalizedSaleStatus === "confirmed") {
      ...
      const stockError = await validateConfirmedStock(sheets, validItems); if (stockError) return NextResponse.json({ error: stockError }, { status: 409 });
    }
