...
function buildDashboardReport(data: DashboardData, activity: ActivityItem[]): ProfessionalReport {
  const attentionItems = buildAttentionItems(data, activity);
  const activityRows = activity.slice(0, 8).map((item) => ({
    Activity: item.title || "Activity",
    Module: item.module || "Not specified",
    Actor: item.actor || "System",
    Time: item.time || "Not specified",
    Summary: item.note || "No note recorded",
  }));

  return {
    title: "Dashboard Operations Report",
    generatedBy: "Main POS Assistant Agent",
    mode: "Safe Read-Only",
    sources: ["/api/dashboard", "/api/recent-activity"],
    ...
    summaryCards: [
      { label: "Total Sales", value: peso(data.totalSales), helper: "Current sales total" },
      { label: "Total Expenses", value: peso(data.totalExpenses), helper: "Current expense total" },
      { label: "Net Gain", value: peso(data.netGain), helper: "Sales less tracked costs" },
      { label: "Sellable Units", value: numberText(data.sellableUnits), helper: "Units available to sell" },
      { label: "Actual On Hand", value: numberText(data.actualOnHand), helper: "Current physical stock" },
      { label: "Recent Activity", value: numberText(activity.length), helper: "Activity records returned" },
    ],
    sections: [
      {
        title: "Operational Attention Items",
        description: "Items that should be reviewed by admin or operations.",
        columns: ["Priority", "Finding"],
        rows: attentionItems.map((item, index) => ({ Priority: index === 0 && item.startsWith("No critical") ? "Normal" : "Review", Finding: item })),
      },
      {
        title: "Recent Activity Snapshot",
        description: "Latest operational activity returned by the activity API.",
        columns: ["Activity", "Module", "Actor", "Time", "Summary"],
        rows: activityRows,
        emptyMessage: "No recent activity was returned by the activity API.",
      },
    ],
    recommendedActions: [
      "Review dashboard totals against the source Google Sheets before finalizing daily reports.",
      "Confirm recent activity entries are expected and not accidental test transactions.",
      "Monitor sellable units and actual on-hand units before accepting large sales commitments.",
    ],
...
function buildActivityAnswer(activity: ActivityItem[]) {
  ...
  return ["Main POS Assistant Agent checked recent activity in read-only mode.", "", rows.length ? rows.join("\n") : "No recent activity returned by the activity API.", "", "Safe-mode note: I only read recent activity. I did not modify activity records."].join("\n");
}

function buildActivityReport(activity: ActivityItem[]): ProfessionalReport {
  const moduleCounts = activity.reduce<Record<string, number>>((counts, item) => {
    const module = item.module || "Unspecified";
  ...
  return {
    ...
    generatedBy: "Main POS Assistant Agent",
    mode: "Safe Read-Only",
    sources: ["/api/recent-activity"],
    executiveSummary: activity.length ? `The system returned ${numberText(activity.length)} recent activity record(s). The latest recorded activity is ${activity[0]?.title || "not specified"}.` : "No recent activity was returned by the activity API.",
    summaryCards: [
      { label: "Activity Records", value: numberText(activity.length), helper: "Recent activity returned" },
      { label: "Modules", value: numberText(Object.keys(moduleCounts).length), helper: "Modules represented" },
      { label: "Latest Actor", value: activity[0]?.actor || "N/A", helper: "Actor on latest record" },
    ],
    sections: [
      { title: "Activity Timeline", description: "Recent operational records, sorted by the source API.", columns: ["Activity", "Action", "Module", "Reference", "Actor", "Time"], rows: activityRows, emptyMessage: "No activity records were returned." },
      { title: "Activity by Module", description: "Count of recent activity records grouped by module.", columns: ["Module", "Records"], rows: moduleRows, emptyMessage: "No module activity summary is available." },
    ],
    recommendedActions: ["Review recent sales and payment activity to confirm each transaction is expected.", "Investigate unusual cancel, reset, or edit events before closing the daily review.", "Use this activity report as a quick audit trail before deployment or end-of-day reporting."],
    systemNote: "This activity report was generated in safe read-only mode. No activity, sales, payment, inventory, customer, supplier, or user records were modified.",
  };
}

function buildQaAnswer() {
  return ["Testing / QA Agent prepared a safe regression checklist.", "", "1. Confirm dashboard loads without console/API errors.", "2. Confirm the Agent Test Panel shows 7 registered agents.", "3. Confirm Ask Realights AI returns a dashboard summary.", "4. Test Add Delivery and verify dashboard totals still refresh.", "5. Test sales, inventory, customer, supplier, and report pages after deployment.", "6. Confirm production writes remain disabled for agents.", "7. Confirm the before-agents restore marker still exists."].join("\n");
}

function buildQaReport(): ProfessionalReport {
  return {
    title: "Release Readiness QA Report",
    generatedBy: "Testing / QA Agent",
    mode: "Safe Read-Only",
    sources: ["Agent registry", "Dashboard UI", "Manual QA checklist"],
    executiveSummary: "The QA agent prepared a regression checklist for validating the dashboard, agent panel, inventory analysis, sales flow, delivery flow, reports, and safe-mode restrictions after deployment.",
    summaryCards: [
      { label: "QA Scope", value: "7 Areas", helper: "Dashboard, sales, inventory, customers, suppliers, reports, AI" },
      { label: "Risk Level", value: "Low", helper: "Read-only agent changes" },
      { label: "Writes", value: "Disabled", helper: "Agent write actions remain blocked" },
      { label: "Restore Marker", value: "Required", helper: "Before risky changes" },
    ],
    sections: [
      {
        title: "Core Regression Checklist",
        description: "Manual checks recommended before marking the deployment stable.",
        columns: ["Area", "Test", "Expected Result", "Status"],
        rows: [
          { Area: "Dashboard", Test: "Load dashboard and refresh KPIs", "Expected Result": "Dashboard loads without API or console errors", Status: "Pending Manual Check" },
          { Area: "AI Agents", Test: "Confirm Agent Test Panel shows 7 agents", "Expected Result": "All agents show Ready status", Status: "Pending Manual Check" },
          { Area: "Ask Realights AI", Test: "Ask dashboard and inventory prompts", "Expected Result": "Professional reports render correctly", Status: "Pending Manual Check" },
          { Area: "Delivery", Test: "Open Add Delivery flow", "Expected Result": "Delivery page opens and dashboard can refresh after changes", Status: "Pending Manual Check" },
          { Area: "Sales", Test: "Review sales create/update/cancel flow", "Expected Result": "Sales records remain consistent", Status: "Pending Manual Check" },
          { Area: "Inventory", Test: "Review stock and reorder report", "Expected Result": "Inventory report matches inventory page totals", Status: "Pending Manual Check" },
          { Area: "Safety", Test: "Confirm Writes Disabled indicators", "Expected Result": "AI agents cannot modify production records", Status: "Pending Manual Check" },
        ],
      },
    ],
    recommendedActions: ["Run the checklist after every Vercel deployment that changes agent behavior or dashboard data display.", "Compare AI report numbers against the dashboard and inventory pages before relying on the report operationally.", "Keep write actions disabled until manual QA passes consistently across sales, inventory, customers, suppliers, and reports."],
    systemNote: "This QA report is advisory and read-only. It does not execute automated browser tests, modify records, deploy code, restore backups, or mark production as approved automatically.",
  };
}

function buildGuardianAnswer() {
  return ["App Guardian Agent status in safe mode:", "", "- Agent registry: active", "- Dashboard read check: available through /api/dashboard", "- Recent activity read check: available through /api/recent-activity", "- Inventory read check: available through /api/inventory and /api/ai/inventory-analysis", "- Production writes: disabled", "- Auto deploy: disabled", "- Auto restore: disabled", "- Restore before risky changes: required", "", "Next upgrade: add a dedicated /api/guardian/health-check endpoint and daily restore-point log."].join("\n");
}

function buildGuardianReport(): ProfessionalReport {
  return {
    title: "App Guardian Safety Report",
    generatedBy: "App Guardian Agent",
    mode: "Monitoring Only",
    sources: ["Agent registry", "/api/dashboard", "/api/recent-activity", "/api/inventory", "/api/ai/inventory-analysis"],
    executiveSummary: "The App Guardian Agent confirms that the agent system is operating in protected safe mode. Read-only analysis is enabled, while production writes, destructive actions, auto-deploy, and auto-restore remain disabled.",
    summaryCards: [
      { label: "Agent Registry", value: "Active", helper: "7 agents registered" },
      { label: "Production Writes", value: "Disabled", helper: "No automatic data mutation" },
      { label: "Auto Restore", value: "Disabled", helper: "Manual approval required" },
      { label: "Auto Deploy", value: "Disabled", helper: "No silent production patching" },
      { label: "Restore Marker", value: "Required", helper: "Before risky changes" },
    ],
    sections: [
      {
        title: "Guardian Health Checks",
        description: "Current safe-mode checks available to the App Guardian layer.",
        columns: ["Check", "Status", "Notes"],
        rows: [
          { Check: "Agent Registry", Status: "Ready", Notes: "All 7 agents registered and testable" },
          { Check: "Dashboard Read", Status: "Available", Notes: "Uses /api/dashboard" },
          { Check: "Recent Activity Read", Status: "Available", Notes: "Uses /api/recent-activity" },
          { Check: "Inventory Read", Status: "Available", Notes: "Uses /api/inventory and /api/ai/inventory-analysis" },
          { Check: "Production Writes", Status: "Blocked", Notes: "No agent writes are enabled" },
          { Check: "Auto Restore", Status: "Blocked", Notes: "Manual admin approval required" },
          { Check: "Auto Deploy", Status: "Blocked", Notes: "No silent deployment actions" },
        ],
      },
    ],
    recommendedActions: ["Add a dedicated /api/guardian/health-check endpoint for deeper API and Google Sheets checks.", "Add the daily restore-point log before enabling any future write-capable agent action.", "Keep auto-deploy and auto-restore disabled until explicit admin approval workflows are implemented."],
    systemNote: "This Guardian report was generated in monitoring-only mode. It did not patch code, deploy, rollback, restore, or modify production records.",
  };
}
