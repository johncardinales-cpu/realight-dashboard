...
function keysForPayment(r: string[]) {
  return uniq([saleKey(r[1], r[2], r[11]), txt(r[11]), txt(r[2]), txt(r[1])]);
}
...
function paymentMap(paymentRows: string[][]) {
  ...
  paymentRows.slice(1).forEach((r) => {
    ...
    keysForPayment(r).forEach((k) => m.set(k, round((m.get(k) || 0) + amount)));
  });
  return m;
}
...
function parseSales(rows: string[][], paymentRows: string[][]) {
  ...
  rows.slice(1).forEach((r) => {
    ...
    const cur = grouped.get(key) || {
      key, saleId, saleDate: normDate(r[0]), salesRefNo, groupRef,
      customerName: txt(r[2]), customerId: txt(r[33]),
      productSubtotalPhp: 0, deliveryFeePhp: 0, installationFeePhp: 0, otherChargePhp: 0,
      discountPhp: 0, taxAmountPhp: 0, totalSalePhp: 0, grossProfitPhp: 0,
      salesPaid: 0, salesBalance: 0, tendered: 0, changeDuePhp: 0,
      initialPaymentMethod: txt(r[15]) || "Unspecified", transactionRef: txt(r[18]), cashierName: txt(r[19]),
      linkedExpensesPhp: 0, saleStatus,
    };
...
function byMethod(entries: any[]) {
  const m = new Map<string, number>();
  entries.forEach((e) => m.set(e.method || "Unspecified", round((m.get(e.method || "Unspecified") || 0) + e.amount)));
  return Array.from(m.entries()).map(([method, amount]) => ({ method, amount })).sort((a, b) => b.amount - a.amount);
}

function byCategory(entries: any[]) {
  const m = new Map<string, number>();
  entries.forEach((e) => m.set(e.category || "Uncategorized", round((m.get(e.category || "Uncategorized") || 0) + e.amount)));
...
function productMovement(rows: string[][], start: string, end: string) {
  ...
  rows.slice(1).forEach((r) => {
    ...
    cur.qty += qty;
    cur.confirmedQty += qty;
    cur.totalSalePhp += num(r[28] || r[7]);
    cur.grossProfitPhp += num(r[10]);
    m.set(key, cur);
  });
...
function makeSaleLookup(allSales: any[]) {
  ...
  allSales.forEach((s) => uniq([s.key, s.saleId, s.groupRef, s.salesRefNo]).forEach((key) => map.set(key, s)));
  return map;
}
...
export async function getReportPayload(url: URL) {
  ...
  const follow = paymentRows.slice(1).filter((r) => !inactive(r[12]) && num(r[5]) > 0 && inRange(r[0], p.startDate, p.endDate)).map((r) => {
    ...
    const sale = saleLookup.get(key) || saleLookup.get(txt(r[11])) || saleLookup.get(txt(r[2])) || saleLookup.get(txt(r[1]));
    return { date: normDate(r[0]), saleDate: sale?.saleDate || "", salesRefNo: sale?.salesRefNo || txt(r[1]) || txt(r[3]), customerName: sale?.customerName || txt(r[3]) || txt(r[1]), method: txt(r[4]) || "Unspecified", amount: num(r[5]), tenderedAmount: num(r[5]), changeDue: 0, transactionRef: txt(r[6]), cashierName: txt(r[7]), paymentId: txt(r[10]), source: "Payments" };
  });
