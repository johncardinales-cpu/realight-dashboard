...
function salesSummary(salesRows: string[][], paymentRows: string[][]) {
  ...
  salesRows.slice(1).forEach((r) => {
    ...
    if (!finalSale(saleStatus) || inactive(saleStatus) || inactive(paymentStatus)) return;
    ...
    const tenderedRaw = num(r[34]);
    const lineTendered = tenderedRaw > 0 ? tenderedRaw : linePaid;
...
function parseExpenses(rows: string[][]) {
  ...
  return rows.slice(1).map((r) => {
    ...
    return {
      ...
      reference: m["Reference No."] || m["Reference"] || "",
...
function attachExpenses(sales: any[], expenses: any[]) {
  ...
  expenses.forEach((e) => {
    const r = txt(e.relatedSalesRefNo).toLowerCase();
    if (r && e.amount > 0) byRef.set(r, (byRef.get(r) || 0) + e.amount);
  });
  return sales.map((s) => {
    const linked = [s.salesRefNo, s.groupRef, s.saleId, s.key].map((x) => txt(x).toLowerCase()).filter(Boolean).reduce((sum, r) => sum + (byRef.get(r) || 0), 0);
...
function dailyTrend(sales: any[], expenses: any[], initial: any[], follow: any[]) {
  ...
  const get = (date: string) => {
    if (!days.has(date)) days.set(date, { date, sales: 0, productSubtotal: 0, deliveryFee: 0, installationFee: 0, otherCharge: 0, discount: 0, tax: 0, collections: 0, cashReceived: 0, changeGiven: 0, expenses: 0, linkedExpenses: 0, grossProfit: 0, netProfit: 0, receivables: 0 });
    return days.get(date);
  };
  sales.forEach((s) => {
    const d = get(s.saleDate);
    d.sales += s.totalSalePhp;
    d.productSubtotal += s.productSubtotalPhp;
    d.deliveryFee += s.deliveryFeePhp;
    d.installationFee += s.installationFeePhp;
    d.otherCharge += s.otherChargePhp;
    d.discount += s.discountPhp;
    d.tax += s.taxAmountPhp;
    d.grossProfit += s.grossProfitPhp;
    d.linkedExpenses += s.linkedExpensesPhp || 0;
    d.receivables += s.balancePhp;
  });
  expenses.forEach((e) => { get(e.date).expenses += e.amount; });
  [...initial, ...follow].forEach((e) => {
    const d = get(e.date);
    d.collections += e.amount;
    d.cashReceived += e.tenderedAmount ?? e.amount;
    d.changeGiven += e.changeDue || 0;
  });
...
function productMovement(rows: string[][], start: string, end: string) {
  ...
  rows.slice(1).forEach((r) => {
    if (!finalSale(r[20]) || inactive(r[20]) || inactive(r[11]) || !inRange(r[0], start, end)) return;
    const description = txt(r[3]);
    const specification = txt(r[4]);
    if (!description && !specification) return;
    const k = `${description}|||${specification}`;
    const current = m.get(k) || { description, specification, qty: 0, confirmedQty: 0, totalSalePhp: 0, grossProfitPhp: 0 };
    current.qty += num(r[5]);
    current.confirmedQty += num(r[5]);
    current.totalSalePhp += num(r[28] || r[7]);
    current.grossProfitPhp += num(r[10]);
    m.set(k, current);
  });
...
export async function GET(req: Request) {
  try {
    ...
    const [salesRows, paymentRows, expenseRows, supplierRows] = await Promise.all([
      ...
      read(sheets, `${EXPENSES_SHEET}!A:Z`),
      read(sheets, `${SUPPLIER_COSTS_SHEET}!A:L`),
    ]);
    ...
    const follow = paymentRows.slice(1).filter(activePayment).filter((r) => inRange(r[0], p.startDate, p.endDate)).map((r) => ({ date: normDate(r[0]), method: txt(r[4]) || "Unspecified", amount: num(r[5]), tenderedAmount: num(r[5]), changeDue: 0 }));
    ...
    const grossProfit = sum(periodSales, "grossProfitPhp");

    return NextResponse.json({
      reportDate: url.searchParams.get("date") || today(),
      mode: p.mode,
      startDate: p.startDate,
      endDate: p.endDate,
      summary: {
        productSubtotalPhp: sum(periodSales, "productSubtotalPhp"),
        deliveryFeePhp: sum(periodSales, "deliveryFeePhp"),
        installationFeePhp: sum(periodSales, "installationFeePhp"),
        otherChargePhp: sum(periodSales, "otherChargePhp"),
        discountPhp: sum(periodSales, "discountPhp"),
        taxAmountPhp: sum(periodSales, "taxAmountPhp"),
        grandTotalPhp: sum(periodSales, "totalSalePhp"),
        totalSalesToday: sum(periodSales, "totalSalePhp"),
        confirmedSalesToday: sum(periodSales, "totalSalePhp"),
        grossProfitToday: grossProfit,
        expensesToday: expensesTotal,
        linkedExpensesToday: sum(periodSales, "linkedExpensesPhp"),
        unlinkedExpensesToday: periodExpenses.filter((e) => !txt(e.relatedSalesRefNo)).reduce((a, e) => a + e.amount, 0),
        netProfitToday: grossProfit - expensesTotal,
        ...
        netCashAfterChangeToday: cashReceived - changeGiven,
        newReceivablesToday: sum(periodSales, "balancePhp"),
        endingReceivables: sum(saleSummaries, "balancePhp"),
        dailySaleCount: periodSales.length,
        paymentStatusCounts: periodSales.reduce((a: Record<string, number>, s) => { a[s.paymentStatus] = (a[s.paymentStatus] || 0) + 1; return a; }, {}),
      },
      accountingBreakdown: {
        productSubtotalPhp: sum(periodSales, "productSubtotalPhp"),
        deliveryFeePhp: sum(periodSales, "deliveryFeePhp"),
        ...
        taxAmountPhp: sum(periodSales, "taxAmountPhp"),
        grandTotalPhp: sum(periodSales, "totalSalePhp"),
        grossProfitPhp: grossProfit,
        linkedExpensesPhp: sum(periodSales, "linkedExpensesPhp"),
        totalExpensesPhp: expensesTotal,
        netProfitPhp: grossProfit - expensesTotal,
      },
      collectionsByMethod: byMethod([...initial, ...follow]),
      cashByMethod: byMethod([...initial, ...follow].map((x) => ({ method: x.method, amount: x.tenderedAmount ?? x.amount }))),
      expensesByCategory: byCategory(periodExpenses),
      dailyTrend: dailyTrend(periodSales, periodExpenses, initial, follow),
      productMovement: productMovement(salesRows, p.startDate, p.endDate),
      dailySales: periodSales.map((s) => ({
        saleDate: s.saleDate,
        salesRefNo: s.salesRefNo,
        customerName: s.customerName,
        customerId: s.customerId,
        productSubtotalPhp: s.productSubtotalPhp,
        deliveryFeePhp: s.deliveryFeePhp,
        installationFeePhp: s.installationFeePhp,
        otherChargePhp: s.otherChargePhp,
        discountPhp: s.discountPhp,
        taxAmountPhp: s.taxAmountPhp,
        ...
        grossProfitPhp: s.grossProfitPhp,
        linkedExpensesPhp: s.linkedExpensesPhp || 0,
        netProfitPhp: s.netProfitPhp,
        paymentStatus: s.paymentStatus,
        saleStatus: s.saleStatus,
      })),
      dailyExpenses: periodExpenses,
      linkedExpenses: periodExpenses.filter((e) => txt(e.relatedSalesRefNo)),
      openReceivables: saleSummaries.filter((s) => s.balancePhp > 0).map((s) => ({
        saleDate: s.saleDate,
        salesRefNo: s.salesRefNo,
        customerName: s.customerName,
        totalSalePhp: s.totalSalePhp,
