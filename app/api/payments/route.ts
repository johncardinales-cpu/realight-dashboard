...
const PAYMENT_HEADERS = ["Payment Date","Sales Ref No.","Group Ref","Customer Name","Payment Method","Amount Paid (PHP)","Transaction Ref","Cashier Name","Notes","Created At","Payment ID","Sale ID"];
...
function saleKey(salesRefNo: string, groupRef: string, saleId?: string) { return saleId || groupRef || salesRefNo; }
...
async function readSalesRows(sheets: any) { const response = await sheets.spreadsheets.values.get({ spreadsheetId: SHEET_ID, range: `${SALES_SHEET}!A:AH` }); return (response.data.values || []) as string[][]; }
async function readPaymentRows(sheets: any) { await ensureSheetExists(sheets, PAYMENTS_SHEET, PAYMENT_HEADERS); const response = await sheets.spreadsheets.values.get({ spreadsheetId: SHEET_ID, range: `${PAYMENTS_SHEET}!A:L` }); return (response.data.values || []) as string[][]; }
...
function buildPaymentTotals(paymentRows: string[][]) {
  ...
  paymentRows.slice(1).forEach((row) => {
    ...
    const saleId = String(row[11] || "").trim();
    const amount = toNumber(row[5]);
    if (amount <= 0) return;
    [saleKey(salesRefNo, groupRef, saleId), saleId, groupRef, salesRefNo].filter(Boolean).forEach((key) => {
...
async function updateSalePaymentFields(sheets: any, salesRows: string[][], key: string, totalPaid: number, paymentStatus: string) {
  const matchKeys = (row: string[]) => [saleKey(String(row[1] || "").trim(), String(row[14] || "").trim(), String(row[22] || "").trim()), String(row[22] || "").trim(), String(row[14] || "").trim(), String(row[1] || "").trim()];
...
export async function POST(req: Request) {
  try {
    ...
    const key = String(body?.key || saleKey(salesRefNo, groupRef, saleId)).trim();
    ...
    await sheets.spreadsheets.values.append({ spreadsheetId: SHEET_ID, range: `${PAYMENTS_SHEET}!A:L`, valueInputOption: "USER_ENTERED", insertDataOption: "INSERT_ROWS", requestBody: { values: [[paymentDate, summary.salesRefNo, summary.groupRef, summary.customerName, paymentMethod, amountPaidPhp, transactionRef, cashierName, notes, new Date().toISOString(), paymentId, summary.saleId || ""]] } });
