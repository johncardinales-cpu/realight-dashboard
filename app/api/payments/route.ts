...
export async function GET() {
  try {
    ...
    const [salesRows, paymentRows] = await Promise.all([
      readSalesRows(sheets),
      readPaymentRows(sheets),
    ]);
    ...
    return NextResponse.json(
      { error: error?.message || String(error) || "Failed to load payment summaries" },
      { status: 500 }
    );
  }
}
...
export async function POST(req: Request) {
  try {
    ...
    if (!key) {
      return NextResponse.json({ error: "Sales Ref No. is required" }, { status: 400 });
    }
    ...
    if (!summary) {
      return NextResponse.json({ error: "Sale reference was not found" }, { status: 404 });
    }

    if (amountPaidPhp > summary.balancePhp) {
      return NextResponse.json(
        { error: `Payment exceeds balance. Remaining balance is ₱${summary.balancePhp.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}` },
        { status: 400 }
      );
    }
    ...
    return NextResponse.json({
      ...
      salesRefNo: summary.salesRefNo,
      totalSalePhp: summary.totalSalePhp,
      totalPaidPhp: newTotalPaid,
      balancePhp: newBalance,
      paymentStatus: newPaymentStatus,
    });
  } catch (error: any) {
    console.error("PAYMENTS POST ERROR:", error);
    return NextResponse.json(
      { error: error?.message || String(error) || "Failed to save payment" },
      { status: 500 }
    );
  }
}
