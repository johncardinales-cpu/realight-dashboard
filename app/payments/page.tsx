...
function StatusPill({ value }: { value: string }) { const n = value.toLowerCase(); const color = n === "paid" || n === "active" || n === "confirmed" ? "bg-emerald-50 text-emerald-700" : n === "partial" ? "bg-amber-50 text-amber-700" : n === "unpaid" || n === "pending" || isInactive(value) ? "bg-rose-50 text-rose-700" : "bg-slate-100 text-slate-700"; return <span className={`rounded-full px-2.5 py-1 text-xs font-bold ${color}`}>{value}</span>; }
...
export default function PaymentsPage() {
  ...
  async function loadPayments() {
    const [summaryRes, historyRes] = await Promise.all([fetch(`/api/payments?t=${Date.now()}`, { cache: "no-store" }), fetch(`/api/payments?history=1&t=${Date.now()}`, { cache: "no-store" })]);
    const summaryData = await summaryRes.json(); const historyData = await historyRes.json();
    ...
    setRows(Array.isArray(summaryData) ? summaryData : []); setHistory(Array.isArray(historyData) ? historyData : []);
  }
  ...
  function changeHistoryMode(mode: HistoryMode) { setHistoryMode(mode); if (mode !== "custom") { const range = periodRange(mode, historyDate); setHistoryStart(range.start); setHistoryEnd(range.end); } }
  function changeHistoryDate(value: string) { setHistoryDate(value); if (historyMode !== "custom") { const range = periodRange(historyMode, value); setHistoryStart(range.start); setHistoryEnd(range.end); } }
