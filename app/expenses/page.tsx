    ...
    <section className="space-y-6">
        ...
        <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          <div>
            ...
            <p className="mt-1 max-w-3xl text-sm leading-6 text-slate-600">
              Record business expenses, fees, taxes, delivery costs paid by the company, fuel, labor, and miscellaneous costs.
            </p>
          </div>
      ...
      <form onSubmit={submitExpense} className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
        ...
        <div className="grid gap-4 md:grid-cols-3">
          ...
          <input className="rounded-xl border border-slate-300 px-3 py-2" type="number" step="0.01" min="0" placeholder="Base Amount" value={baseAmount} onChange={(event) => setBaseAmount(Number(event.target.value))} required />
          <input className="rounded-xl border border-slate-300 px-3 py-2" type="number" step="0.01" min="0" placeholder="Tax / VAT / Fee" value={taxFee} onChange={(event) => setTaxFee(Number(event.target.value))} />
          <input className="rounded-xl border border-slate-300 bg-slate-50 px-3 py-2 font-semibold text-slate-800" value={`Total: ${currency(totalAmount)}`} readOnly />
          ...
          <input className="rounded-xl border border-slate-300 px-3 py-2 md:col-span-2" placeholder="Notes" value={notes} onChange={(event) => setNotes(event.target.value)} />
        </div>
          ...
          <table className="w-full text-sm">
            <thead className="bg-slate-100 text-slate-700">
              <tr>
                ...
                <th className="px-4 py-3 text-left font-medium whitespace-nowrap">Base</th>
                <th className="px-4 py-3 text-left font-medium whitespace-nowrap">Tax/Fee</th>
            ...
            <tbody>
              {rows.map((row, index) => (
                <tr key={`${row.Source}-${row.Reference}-${row.ExpenseID || index}`} className="border-t border-slate-100 align-top">
                  ...
                  <td className="px-4 py-3 text-slate-700">{currency(Number(row.BaseAmount ?? row.Amount) || 0)}</td>
                  <td className="px-4 py-3 text-slate-700">{currency(Number(row.TaxFee) || 0)}</td>
