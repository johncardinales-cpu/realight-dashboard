  ...
  async function submitExpense(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const res = await fetch("/api/expenses", {
        ...
        body: JSON.stringify({ expenseDate, category, description, baseAmount, taxFee, totalAmount, paymentMethod, referenceNo, relatedSalesRefNo, customerName, payee, notes, actor: payee || customerName || "Admin" }),
      });
    ...
    <section className="space-y-6">
      ...
      <form onSubmit={submitExpense} className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
        <div className="mb-4">
          ...
          <p className="mt-1 text-sm text-slate-600">Base amount is the expense before tax/fees. Use Customer / Expense For to identify who the expense belongs to.</p>
        </div>
