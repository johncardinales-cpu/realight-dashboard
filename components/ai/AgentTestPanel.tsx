        ...
        <div className="mt-4 border-t border-slate-100 pt-4">
          <div className="mb-4 grid grid-cols-2 gap-2 xl:grid-cols-4">
            <div className="rounded-xl border border-slate-200 bg-slate-50 p-3">
              <p className="text-[10px] font-bold uppercase tracking-wide text-slate-400">System Mode</p>
              <p className="mt-1 text-sm font-semibold text-slate-950">{status?.mode || "safe"}</p>
            </div>
            <div className="rounded-xl border border-slate-200 bg-slate-50 p-3">
              <p className="text-[10px] font-bold uppercase tracking-wide text-slate-400">Registered</p>
              <p className="mt-1 text-sm font-semibold text-slate-950">{status?.agents?.length ?? "..."}</p>
            </div>
            <div className="rounded-xl border border-slate-200 bg-slate-50 p-3">
              <p className="text-[10px] font-bold uppercase tracking-wide text-slate-400">Writes</p>
              <p className="mt-1 text-sm font-semibold text-slate-950">{status?.writeActionsEnabled ? "Enabled" : "Disabled"}</p>
            </div>
            <div className="rounded-xl border border-slate-200 bg-slate-50 p-3">
              <p className="text-[10px] font-bold uppercase tracking-wide text-slate-400">Auto Restore</p>
              <p className="mt-1 text-sm font-semibold text-slate-950">{status?.autoRestoreEnabled ? "Enabled" : "Disabled"}</p>
            </div>
          </div>
                ...
                <div key={agent.id} className="rounded-2xl border border-slate-200 bg-white p-3 shadow-sm transition hover:border-emerald-200 hover:shadow-md">
                  <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                    <div className="min-w-0">
                      <div className="flex flex-wrap items-center gap-2">
                        ...
                        <span className="rounded-full bg-emerald-50 px-2 py-0.5 text-[9px] font-bold uppercase tracking-wide text-emerald-700">{agent.status}</span>
                        <span className="rounded-full bg-slate-100 px-2 py-0.5 text-[9px] font-bold uppercase tracking-wide text-slate-600">{agent.mode}</span>
                      </div>
                      <p className="mt-1 truncate text-xs font-semibold text-slate-500">{agent.title}</p>
                    </div>

                    <button
                      type="button"
                      onClick={() => setExpandedAgentId(isExpanded ? null : agent.id)}
                      className="rounded-xl border border-slate-200 bg-white px-3 py-1.5 text-xs font-semibold text-slate-700 shadow-sm transition hover:border-emerald-200 hover:bg-emerald-50 hover:text-emerald-700"
                    >
                  ...
                  {isExpanded ? (
                    <div className="mt-3 border-t border-slate-100 pt-3">
                      <p className="text-xs leading-5 text-slate-600">{agent.summary}</p>
                      <div className="mt-3 rounded-xl bg-slate-50 p-3">
                        <p className="text-[10px] font-bold uppercase tracking-wide text-slate-400">Sample Prompt</p>
                        <p className="mt-1 text-xs font-semibold text-slate-700">{agent.samplePrompts[0]}</p>
                      </div>
                      <button onClick={() => testAgent(agent)} disabled={testingAgentId === agent.id} className="mt-3 rounded-xl bg-emerald-600 px-3 py-1.5 text-xs font-semibold text-white shadow-sm shadow-emerald-600/20 transition hover:bg-emerald-700 disabled:cursor-not-allowed disabled:bg-slate-300">
          ...
          {testResponse ? (
            <div className="mt-4 rounded-2xl border border-emerald-200 bg-emerald-50 p-4">
              <p className="text-xs font-bold uppercase tracking-wide text-emerald-700">Latest Test Result</p>
              <h3 className="mt-1 text-lg font-semibold text-slate-950">{testResponse.agentName}</h3>
